...
(def reqhead (meth path hds)
  ...
  ; "Transfer-Encoding: chunked" (and we don't, at least yet)
  (pr-headers hds))
...
(= httpd-hds*    (obj Server        "http.arc"
   		      Content-Type  "application/xhtml+xml"  ; set encoding in your HTML
		      Connection    "closed")
   stop-httpd*	  nil
...
(def handle-req (in out ip)
  (after 
    (let req (read-req in)
      (aif (alref req!hds "X-Real-IP") (= req!ip it) (= req!ip ip))
      (read-body req in)
